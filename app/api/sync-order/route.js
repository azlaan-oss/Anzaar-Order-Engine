...
export async function POST(req) {
  try {
    ...
    if (!process.env.GOOGLE_SHEETS_CLIENT_EMAIL || !process.env.GOOGLE_SHEETS_PRIVATE_KEY) {
       console.error("CRITICAL: Google Sheets Credentials missing from .env.local");
       return new Response(JSON.stringify({ success: false, error: "Cloud Credentials missing" }), { status: 500 });
    }
    ...
    if (!spreadsheetId) {
      console.error("CRITICAL: Spreadsheet ID missing");
      return new Response(JSON.stringify({ success: false, error: "Spreadsheet ID missing" }), { status: 500 });
    }
    ...
    const productsSum = order.items.reduce((sum, item) => sum + (item.price * item.quantity), 0);

    const values = [
      [
        new Date(order.timestamp).toLocaleDateString('en-GB'), // Date
        ...
        "App View",                                           // Product Photo
        productsSum,                                          // Price
        order.totals.delivery,                                // Delivery Charge
        order.totals.total,                                   // Total
        order.payment.advancePaid ? order.payment.amount : 0, // Advance
        order.payment.method === 'Bkash' ? order.payment.transactionId : (order.payment.method === 'Nagad' ? `Nagad: ${order.payment.transactionId}` : (order.payment.transactionId || '')), // Payment Ref
    ...
    console.log(`📡 Attempting to Sync Order #${order.orderId} to Sheet: ${spreadsheetId}`);
    ...
    console.log(`✅ Sync Success for #${order.orderId}`);
    return new Response(JSON.stringify({ success: true, data: response.data }), { status: 200 });

  } catch (error) {
    console.error('❌ Google Sheets Sync Error:', error.message, error.stack);

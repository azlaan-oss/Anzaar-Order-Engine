...
export const moveToTrash = async (collectionName, docId) => {
  try {
    console.log(`[TRASH] Move initiated for ${docId} in ${collectionName}`);
    const safeId = String(docId).trim();
    let docRef = doc(db, collectionName, safeId);
    let docSnap = await getDoc(docRef);
    
    // Fallback: If not found by ID, try searching by common identifier fields
    if (!docSnap.exists()) {
      console.warn(`[TRASH] Doc ID ${docId} not found directly. Searching for alternate match...`);
      const q = query(collection(db, collectionName), where("id", "==", docId));
      const searchSnap = await getDocs(q);
      
      if (!searchSnap.empty) {
        docRef = searchSnap.docs[0].ref;
        docSnap = searchSnap.docs[0];
        console.log(`[TRASH] Found match via internal id field at ${docRef.id}`);
      } else {
        // Try searching by name or SKU if it's a product
        const q2 = query(collection(db, collectionName), where("name", "==", docId));
        const searchSnap2 = await getDocs(q2);
        if (!searchSnap2.empty) {
          docRef = searchSnap2.docs[0].ref;
          docSnap = searchSnap2.docs[0];
          console.log(`[TRASH] Found match via name field at ${docRef.id}`);
        } else {
           throw new Error(`Item not found in ${collectionName} vault (tried ID/Field searches)`);
        }
      }
    }
